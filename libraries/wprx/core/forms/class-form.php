...
use Wprx\Utils\Object_Factory as Object_Factory;
    ...
    public function output( $group = null, $override_options = array() ) {
        ...
        if ( is_array( $fields ) && count( $fields) > 0 ) {
            ...
            foreach ( $fields as $field ) {
                ...
                $output[] = sprintf(
                    '<tr><th scope="row"><label for="%1$s" class="wprx-form-tooltip" title="%4$s" data-tooltip="%4$s">%2$s</label></th><td>%3$s</td></tr>',
                    $field_id,
                    $field_label,
                    $field->output(),
